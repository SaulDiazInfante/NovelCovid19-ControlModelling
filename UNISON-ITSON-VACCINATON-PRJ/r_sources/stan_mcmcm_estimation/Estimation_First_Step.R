...
#             that is, from February 19 to March 23
...
#
#
covid19_CDMX_MX_data <- fread("Data_CDMX_Estado_0309_v1.csv",
                               select = c("Date",
                                          "Onset_PD",
                                          "Cumulative_Onset"))
...
m1 <- stan_model("vaccination_v1.stan")
#
#### mcmcm parameters ####
n_chains  <- 5
n_warmups <- 500
...
#### Set initial values:
ini_1 <- function(){
  list(theta = c(rnorm(1, mean = 1.0, sd = 0.13),
                 rnorm(1, mean = 1.0, sd = 0.13),
                 runif(1, 0.05, 0.25)
  ),
  E0  = runif(1, 2, 20),
  Ia0 = runif(1, 2, 10)
...
#save.image(file = "Data2303_v12_70.RData")
#
...
pairs(nuts_fit_1,
      ...
      labels = c("beta_a", "beta_s", "rho", "R0", "E0", "Ia0"),
      cex.labels = 1.5,
      font.labels = 9,
...
write.table(Mat_theta, file = "theta_values_v1.csv", sep = ",", 
            col.names = c("beta_a", "beta_s", "rho"),
            row.names = FALSE)

write.table(Mat_IC, file = "IC_v1.csv", sep = ",", 
...
Data_CDMXEST <- read.csv("Data_CDMX_Estado_0309_v1.csv", head = TRUE)
...
ode1 <- function(t, x1, par1){ 
  ...
  dS   = mu_h * N_star - FOI_1 - mu_h * S + delta_r * R
...
MSt  <- matrix(data = 0, nrow = length(vect), ncol = 5)
...
for(i in vect){
  par1 = c(beta_a = theta_val[i,1], beta_s = theta_val[i,2], delta_e = 1/5.1,
           ...
           mu_h = 0.000039139, delta_r = 1/365)
